...
file inside a Git repository. Rust buffers add `;dr` (Rust Debuggables) and the `:RustLsp`
command from rustaceanvim; note that rustaceanvim also tries to bind `;cR` to its grouped code
actions, but LazyVim's own `;cR` (Rename File) wins — use `;ca` or `:RustLsp codeAction`.
